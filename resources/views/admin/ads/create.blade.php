    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl font-bold text-gray-800">
                {{ isset($ad) ? 'Edit Iklan' : 'Buat Iklan Baru' }}
            </h1>
        </div>

        <div class="bg-white rounded-2xl shadow-sm p-6">
            <form action="{{ isset($ad) ? route('admin.ads.update', $ad) : route('admin.ads.store') }}"
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">JPG, PNG, WebP. Maks 2MB. Rekomendasi: 728×90px (leaderboard) atau 320×100px (mobile)</p>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        @foreach(['footer_sticky' => ['📌 Footer Sticky', 'Menempel di bawah layar'], 'in_content' => ['📄 In-Content', 'Di antara konten halaman']] as $val => [$label, $desc])
                        <label class="relative cursor-pointer">
                            <input type="radio" name="position" value="{{ $val }}"
                                   {{ old('position', $ad->position ?? 'footer_sticky') === $val ? 'checked' : '' }}
                                   class="peer sr-only">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Pilih "Semua Halaman" untuk tampil di mana saja</p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="datetime-local" name="starts_at"
                               value="{{ old('starts_at', isset($ad) && $ad->starts_at ? $ad->starts_at->format('Y-m-d\TH:i') : '') }}"
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="ends_at"
                               value="{{ old('ends_at', isset($ad) && $ad->ends_at ? $ad->ends_at->format('Y-m-d\TH:i') : '') }}"
                               class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
                ...
                <div class="flex gap-3 pt-2">
                    <button type="submit"
                        ...
                        {{ isset($ad) ? 'Simpan Perubahan' : 'Buat Iklan' }}
                    </button>
                    <a href="{{ route('admin.ads.index') }}"
