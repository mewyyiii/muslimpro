...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-700">Edit Iklan</h2>
    </div>
        ...
        <form method="POST" action="{{ route('admin.ads.update', $ad) }}" enctype="multipart/form-data"
            ...
            <div class="mb-5">
                <label class="block text-sm font-medium text-gray-700 mb-1.5">
                    Gambar <span class="text-gray-400 font-normal">(maks. 512KB, format: jpg/png/webp — kosongkan jika tidak diganti)</span>
                </label>
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-teal-400 transition"
                    ...
                    <template x-if="preview">
                        <div>
                            ...
                            <label for="imageInput" class="text-xs text-teal-600 cursor-pointer hover:underline">
                                Ganti gambar
                            </label>
                        </div>
                    </template>

                    {{-- Gambar saat ini (sebelum diganti) --}}
                    <template x-if="!preview">
                        <div>
                            <img src="{{ $ad->image_url }}" alt="{{ $ad->title }}"
                                 class="max-h-40 mx-auto rounded-lg object-contain mb-3">
                            <label for="imageInput" class="text-xs text-teal-600 cursor-pointer hover:underline">
                                Klik atau drag & drop untuk ganti gambar
                            </label>
                        </div>
                    </template>
                </div>
            ...
            <div class="mb-5">
                ...
                <select name="position"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
                    <option value="in_content"    {{ old('position', $ad->position) === 'in_content'    ? 'selected' : '' }}>Bawah Header</option>
                    <option value="footer_sticky" {{ old('position', $ad->position) === 'footer_sticky' ? 'selected' : '' }}>Bawah Layar</option>
                </select>
                @error('position')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    ...
                    <label class="flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2.5 cursor-pointer hover:border-teal-400 hover:bg-teal-50 transition text-sm" style="display:flex;align-items:center;">
                        <input type="checkbox" name="pages[]" value="{{ $value }}"
                               {{ in_array($value, old('pages', $ad->pages ?? [])) ? 'checked' : '' }}
