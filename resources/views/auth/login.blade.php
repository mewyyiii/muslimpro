...
<head>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .blob { position:absolute; border-radius:50%; filter:blur(60px); opacity:.25; animation:drift 8s ease-in-out infinite; }
        ...
        .pattern-overlay {
            ...
            background-image:
                repeating-linear-gradient(60deg,white 0,white 1px,transparent 1px,transparent 30px),
                repeating-linear-gradient(-60deg,white 0,white 1px,transparent 1px,transparent 30px),
                repeating-linear-gradient(0deg,white 0,white 1px,transparent 1px,transparent 30px);
            background-size:35px 60px;
        }
        .cover-content { position:relative;z-index:2;text-align:center;width:100%;max-width:520px;animation:fadeUp .9s ease-out both; }
        @keyframes fadeUp { from{opacity:0;transform:translateY(24px);}to{opacity:1;transform:translateY(0);} }
        .cover-logo { display:flex;align-items:center;justify-content:center;gap:16px;margin-bottom:32px; }
        .nur-circle { width:64px;height:64px;border-radius:50%;overflow:hidden;flex-shrink:0;animation:floatY 4s ease-in-out infinite; }
        .nur-icon { width:64px;height:64px;display:block;border-radius:50%; }
        @keyframes floatY { 0%,100%{transform:translateY(0);}50%{transform:translateY(-8px);} }
        .cover-brand-name { font-size:32px;font-weight:800;color:white;letter-spacing:-.5px; }
        .deco-top { position:absolute;top:32px;right:32px;opacity:.3; }
        .cover-headline { font-size:clamp(22px,2.8vw,38px);font-weight:800;color:white;line-height:1.2;margin-bottom:16px;letter-spacing:-.5px; }
        .cover-headline span { display:block;color:var(--teal-pale);font-style:italic; }
        .cover-sub { font-size:clamp(13px,1.2vw,15px);color:rgba(255,255,255,.8);line-height:1.7;margin-bottom:28px;max-width:400px;margin-inline:auto; }
        .feature-pills { display:flex;flex-wrap:wrap;gap:10px;justify-content:center;margin-bottom:28px; }
        .pill { display:flex;align-items:center;gap:8px;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.2);border-radius:100px;padding:8px 16px;font-size:13px;font-weight:600;color:white;backdrop-filter:blur(4px);transition:background .2s; }
        .pill:hover { background:rgba(255,255,255,.2); }
        .pill-icon { width:18px;height:18px;display:flex;align-items:center; }
        .arabic-quote { font-family:'Amiri',serif;font-size:22px;color:rgba(255,255,255,.6);border-top:1px solid rgba(255,255,255,.2);padding-top:24px;direction:rtl;letter-spacing:1px; }
        .arabic-quote-trans { font-size:12px;color:rgba(255,255,255,.5);margin-top:6px;direction:ltr;font-style:italic; }
        .deco-circles { position:absolute;bottom:40px;left:40px;display:flex;gap:10px;opacity:.3; }
        ...
        .form-inner { width:100%;max-width:360px;animation:fadeUp .7s .2s ease-out both; }

        /* Mobile logo — hidden on desktop */
        .mobile-logo { display:none; }

        .form-title  { font-size:26px;font-weight:800;color:var(--text-dark);margin-bottom:6px;letter-spacing:-.5px; }
        .form-subtitle { font-size:14px;color:var(--text-soft);margin-bottom:32px; }
        .form-group { margin-bottom:18px; }
        .input-label { display:block;font-size:13px;font-weight:600;color:var(--text-mid);margin-bottom:8px;letter-spacing:.3px; }
        .input-wrapper { position:relative;border-radius:14px;border:1.5px solid #e2e8f0;background:#f8fafc;transition:border-color .25s,box-shadow .25s; }
        .input-wrapper:hover { border-color:var(--teal-mid); }
        .input-wrapper.focused { border-color:var(--teal-deep);box-shadow:0 0 0 3px rgba(13,148,136,.12);background:white; }
        ...
        .eye-toggle { position:absolute;right:14px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--text-soft);transition:color .2s;display:flex;align-items:center; }
        .eye-toggle:hover { color:var(--teal-deep); }
        .eye-toggle svg { width:18px;height:18px; }
        .row-between { display:flex;justify-content:space-between;align-items:center;margin:14px 0 22px;font-size:13px; }
        ...
        .forgot-link { color:var(--teal-deep);text-decoration:none;font-weight:600;transition:color .2s; }
        .forgot-link:hover { color:var(--teal-mid);text-decoration:underline; }
        .login-btn { width:100%;padding:15px;border:none;border-radius:14px;font-size:15px;font-weight:700;font-family:inherit;cursor:pointer;background:linear-gradient(135deg,#0d9488,#14b8a6);color:white;box-shadow:0 6px 20px rgba(13,148,136,.35);transition:all .25s;letter-spacing:.5px; }
        .login-btn:hover  { transform:translateY(-2px);box-shadow:0 10px 28px rgba(13,148,136,.45); }
        ...
        /* ============================================================
           ...
           - card putih terpusat dengan shadow + rounded corners
           - cover panel HILANG sepenuhnya
           - logo NurSteps kecil di atas form di dalam card
        ============================================================ */
        @media (max-width: 860px) {
            ...
            /* Sembunyikan cover panel sepenuhnya */
            .cover-panel { display: none !important; }

            /* Form panel jadi container transparan */
            .form-panel {
                display: block;
                width: 100%;
                max-width: 420px;
            ...
            /* Logo NurSteps kecil di atas form */
            .mobile-logo {
                display: flex;
                align-items: center;
            ...
            .mobile-logo-icon svg { width: 44px; height: 44px; display: block; }
            .mobile-logo-name {
                font-size: 22px; font-weight: 800;
                color: var(--teal-deep); letter-spacing: -.4px;
            }

            .form-title    { font-size: 22px; text-align: center; margin-bottom: 4px; }
            .form-subtitle { font-size: 13px; text-align: center; margin-bottom: 24px; }
            .form-footer   { margin-top: 20px; }
        }

        @media (max-width: 420px) {
            body { padding: 24px 14px 44px; }
            .form-inner { padding: 28px 20px 28px; border-radius: 20px; }
        }
    </style>
    ...
    <script>
        function togglePassword() {
            var i=document.getElementById('password-input'),
                eo=document.getElementById('eye-open'),
                ec=document.getElementById('eye-closed');
...
<body>
                ...
                <div class="nur-circle">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" class="nur-icon">
                        <defs>
                            ...
                            <radialGradient id="cg" cx="52%" cy="48%" r="35%">
                        ...
                        <g clip-path="url(#cc)">
                            ...
                            <rect width="1024" height="1024" fill="url(#cg)"/>
    ...
    <!-- ===== FORM PANEL (kanan desktop / card mobile) ===== -->
    <div class="form-panel">
        <div class="form-inner">

            <!-- Logo NurSteps — hanya tampil di mobile -->
            <div class="mobile-logo">
                <div class="mobile-logo-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024">
                        <defs>
                            <clipPath id="mc"><circle cx="512" cy="512" r="512"/></clipPath>
                            <radialGradient id="mg" cx="50%" cy="45%" r="65%">
                                <stop offset="0%" stop-color="#2ecf8e"/>
                                <stop offset="100%" stop-color="#1aaa72"/>
                            </radialGradient>
                        </defs>
                        <g clip-path="url(#mc)">
                            <rect width="1024" height="1024" fill="url(#mg)"/>
                            <text x="510" y="570" text-anchor="middle" dominant-baseline="middle"
                                font-family="'Noto Naskh Arabic','Traditional Arabic',serif"
                                font-size="430" font-weight="bold" fill="white" direction="rtl">نور</text>
                        </g>
                    </svg>
                </div>
                <span class="mobile-logo-name">NurSteps</span>
            </div>
